...
//插入标题
	...
	$("#submit").on("click",function(){
		...
		$.each(aLi,function(index,opt){
			if($(opt).hasClass("textarea")){
				if(typeof $(opt).attr("data") != "undefined"){
					if($(opt).children("div.editor").hasClass("para")){
						...
						total = total +"<text>" +$(opt).attr("data")+"</text>"
					}
					if($(opt).children("div.editor").hasClass("title")){
						total = total +"<caption>" +$(opt).attr("data")+"</caption>"
					}
			...
			if($(opt).hasClass("img")){
				total = total +"<img>"+$(opt).attr("img")+"</picture>"
				
			}
			if($(opt).hasClass("video")){
				total = total +"<video thum='"+$(opt).attr("video")+"'>"+$(opt).attr("VD")+"</video>"
			}
		})
...
$("#content").html("")
	...
	$("#getBtn").on("click",function(){
		...
		console.log(_to);
		var arr = ["<div>哈哈</div>","<div>喔喔</div>","<div>偶偶</div>"]
		$.each(arr,function(index,opt){
			$("div.editor").append($(opt));
		})
	})
