$(document).mousewheel(function () {
	...
	// var cH=parseInt($("#content").css("height"))-parseInt($("#sidebar").css("height"))-5;
	// console.log(cH);
	// if(h>cH){
	// 	h = cH;
	// }
	// $("#sidebar").css("top",h);
        var cH=parseInt($("#content").css("height"))-350;

    $("#sidebar").css("top",cH+"px");
});
	...
	$("#insertText").on("click",function(e){
		...
		if(_element){
				...
				if($(_element).hasClass("selected")){
					...
					var _div = $("<div></div>").addClass("editor para").html("插入文本").attr("contenteditable","plaintext-only")
					$(_div).appendTo(_con);
				}
...
//插入标题
	$("#insertTitle").on("click",function(e){
		...
		if(_element){
				...
				if($(_element).hasClass("selected")){
					var _con = $("<div></div>").addClass("textarea ct").insertAfter($(_element))
					var _div = $("<div></div>").addClass("editor title").html("插入标题").attr("contenteditable","plaintext-only")
					$(_div).appendTo(_con)
				}
	...
	$("#submit").on("click",function(){
		...
		$.each(aLi,function(index,opt){
			if($(opt).hasClass("textarea")){
				//存文本信息
				var aList =  $(opt).children("div.editor").contents();
				var _html = "";
...
//			var col = arr[i].split("%").slice(1,-1);
			var st = arr[i].substr(2)
			var col = st.split("%").slice(0,-1)
			if( index == "a" ){
				renderTxt(st,col);
			}
			if(index == "b"){
...
//重绘文本
function renderTxt(str,arr){
		
		var _con = $("<div></div>").addClass("textarea ct").attr("data",str).appendTo("#content");
		var _div = $("<div></div>").addClass("editor para").attr("contenteditable","plaintext-only");
		$(_div).appendTo(_con);	
//		console.log(arr)
...
//重绘段落
function renderPara(str,arr){
		
			var _con = $("<div></div>").addClass("textarea ct").attr("data",str).appendTo("#content");
			var _div = $("<div></div>").addClass("editor title").attr("contenteditable","plaintext-only");
...
function renderVideo(str){
	...
	var num1 = day1.getTime();
		...
		$("<div></div>").addClass("video ct IV Video"+num1).attr("video","["+num1+"]").attr("VD",str).appendTo("#content");
...
function renderMob(str){
		var day1 = new Date;
	var num1 = day1.getTime();
	
		$("<div></div>").addClass("video ct IV Video"+num1).attr("video","["+num1+"]").attr("VD",str).appendTo("#content");
		
		$("<video></video>").addClass("video-js vjs-big-play-centered Video"+num1).attr({"preload":"auto","id":"my-video","controls":true}).appendTo("div.Video"+num1);
		
		$("<source>",{src:str}).attr("type","application/x-mpegURL").appendTo("video.Video"+num1);
...
function playVideo(div,url){
	...
	CKobject.embed('assets/ckplayer/ckplayer.swf',div,'ckplayer_a1',
